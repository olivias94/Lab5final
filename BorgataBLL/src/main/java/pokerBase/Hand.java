...
public class Hand extends HandDomainModel {
	...
	public static Hand holdemPicker(Hand Pcards, ArrayList<Card> ccards){
		ArrayList<CardDomainModel>pcards = Pcards.getCards();
		ArrayList<Hand> listh = new ArrayList<Hand>();
		listh.add(new Hand(ccards));
		ArrayList<Card> handcards = new ArrayList<Card>();
		
		
		for(CardDomainModel i:pcards)
		{
			handcards.add((Card) i);
			for(Card k:ccards)
			{
				handcards.add(k);
				for(Card j:ccards)
...
//			Hand.ShowHand(q);
		...
		Collections.sort(listh, Hand.HandRank);
		return listh.get(0);
	}
	...
	private static void ShowHand(Hand x){
		...
		for (CardDomainModel i : x.getCards()){
